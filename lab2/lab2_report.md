...
Был создан сервис в Google Cloud Run с использованием стандартного контейнера gcr.io/cloudrun/hello.
При создании были выбраны минимальные ресурсы:
CPU: 1
Memory: 128 MiB
...
После развертывания сервис получил URL для доступа.
...
5. Анализ логов и метрик
В разделе логов было обнаружено сообщение:
...
Это подтверждает, что контейнер успешно запущен и работает корректно.
Также были проанализированы метрики:
количество запросов
задержка (latency)
